...
export function walletRoutes() {
  ...
  r.post("/deposit", async (req, res) => {
    ...
    const w = await Wallet.findOneAndUpdate(
      { user: uid, blocked: { $ne: true } },
      { $inc: { balance: amount } },
      { new: true }
    )
    if (!w) return res.status(404).json({ message: "wallet_not_found_or_blocked" })
  ...
  r.post("/withdraw", async (req, res) => {
    ...
    const w = await Wallet.findOneAndUpdate(
      { user: uid, blocked: { $ne: true }, balance: { $gte: amount } },
      { $inc: { balance: -amount } },
      { new: true }
    )
    ...
    if (!w) {
      ...
      if (!exists) return res.status(404).json({ message: "wallet_not_found" })
      if (exists.blocked) return res.status(400).json({ message: "wallet_blocked" })
  ...
  r.post("/send", async (req, res) => {
    ...
    if (!receiver) return res.status(404).json({ message: "receiver_not_found" })
    if (receiver._id.toString() === uid) {
      return res.status(400).json({ message: "invalid_receiver" })
    }

    
    const wFrom = await Wallet.findOneAndUpdate(
      { user: uid, blocked: { $ne: true }, balance: { $gte: amount } },
      { $inc: { balance: -amount } },
      { new: true }
    )
    ...
    const wTo = await Wallet.findOneAndUpdate(
      { user: receiver._id, blocked: { $ne: true } },
      { $inc: { balance: amount } },
      { new: true }
    )
    if (!wTo) {
