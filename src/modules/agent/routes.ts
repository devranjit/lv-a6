...
export function agentRoutes(env: { commissionBps: number }) {
  const r = Router()
  ...
  r.get("/transactions", async (req, res) => {
    ...
    const q: any = { agent: new mongoose.Types.ObjectId(agentId) }
    if (type) q.type = type
    if (from || to) {
      q.createdAt = {}
  ...
  r.post("/cashin", async (req, res) => {
    ...
    const w = await Wallet.findOneAndUpdate(
      { user: target._id, blocked: { $ne: true } },
      { $inc: { balance: amount } },
      { new: true }
    )
  ...
  r.post("/cashout", async (req, res) => {
    ...
    if (!target) return res.status(404).json({ message: "user_not_found" })
    const w = await Wallet.findOneAndUpdate(
      { user: target._id, blocked: { $ne: true }, balance: { $gte: amount } },
      { $inc: { balance: -amount } },
      { new: true }
    )
    if (!w) {
