...
export function adminRoutes() {
    ...
    r.get("/users", async (req, res) => {
        ...
        const where = {};
        if (q)
            where.$or = [{ name: { $regex: q, $options: "i" } }, { email: { $regex: q, $options: "i" } }];
        if (role)
            where.role = role;
        ...
        let filtered = data;
        if (status === "blocked")
            filtered = data.filter(d => d.walletBlocked);
        if (status === "active")
            filtered = data.filter(d => !d.walletBlocked);
        if (status === "pending")
            filtered = data.filter(d => d.role === "agent" && d.agentStatus === "pending");
        if (status === "suspended")
            filtered = data.filter(d => d.role === "agent" && d.agentStatus === "suspended");
        if (status === "approved")
            filtered = data.filter(d => d.role === "agent" && d.agentStatus === "approved");
    ...
    r.get("/transactions", async (req, res) => {
        ...
        if (who) {
            ...
            if (ids.length)
                q.$or = [{ fromUser: { $in: ids } }, { toUser: { $in: ids } }, { agent: { $in: ids } }];
            else
                q.$or = [{ _id: new mongoose.Types.ObjectId("000000000000000000000000") }];
        }
        const [items, total] = await Promise.all([
